...
for item in goods.items():
    stores_list = store[item[1]]
    item_name = item[0]
    # Обнуление для нового товара
    total_quantity = 0
    total_cost = 0
    for current_store in stores_list:
        current_quantity = current_store['quantity']
        current_price = current_store['price']
        # Подсчет общей стоимости на текущем складе
        total_cost_in_current_store = current_quantity * current_price
        # Подсчет общего количества
        total_quantity += current_quantity
        total_cost += total_cost_in_current_store
    print(f'{item_name} - {total_quantity} шт, общая стоимость {total_cost} руб')
